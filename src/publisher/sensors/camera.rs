...
impl CameraDataPublisher {
  ...
  fn publish_color(&self, frame_id: &str, data: &Bound<'_, PyBytes>, width: f32, height: f32) -> PyResult<()> {
  ...
  fn publish_depth(&self, frame_id: &str, data: &Bound<'_, PyBytes>, width: f32, height: f32) -> PyResult<()> {
    let header = create_header(frame_id);
    let width = width as u32;
    let height = height as u32;
    let encoding = "32FC1".to_string();
    let step = width * 4;
  ...
  fn publish_annotation(&self, frame_id: &str, data: &Bound<'_, PyBytes>, width: f32, height: f32) -> PyResult<()> {
    let header = create_header(frame_id);
    let width = width as u32;
    let height = height as u32;
    let encoding = "rgba8".to_string();
